...
import { Text, View, Alert, FlatList, SafeAreaView, Dimensions } from 'react-native';
...
export default class MeteorScreen extends Component {
    ...
    getMeteors = () => {
        axios
            .get("https://api.nasa.gov/neo/rest/v1/feed?api_key=KJBhf0W0RrrAPvsb240sJxtuNPHK4O5SL6gGeLXN")
            .then(response => {
    ...
    render() {
        if (Object.keys(this.state.meteors).length === 0) {
            ...
            let meteor_arr = Objects.keys(this.state.meteors).map(meteor_date => {
                return (
